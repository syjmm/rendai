            ...
            <el-table :data="tableData" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="操作" width="350" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button size="small" type="danger" v-if="scope.row.audit == '0' || scope.row.audit == '-1'" @click="handleDelete(scope.$index, scope.row)">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            add(){
                let token = localStorage.getItem('ms_token');
                this.$axios.get('/manager/notifications/uid',{
                    headers: {
                        'Authorization':token,
                    }
                }).then(res=>{
                    this.uid=res.data.data.uid;
                    this.$router.push({path:'/iform',query:{uid:this.uid}})
                })
            ...
            handleShenhe(index,row){
                ...
                this.$axios.post('/manager/notifications/to_audit',
                    qs.stringify({
                        uid:this.tid
                    }),
                    {
            ...
            handlepublish(index,row){
                ...
                let token = localStorage.getItem('ms_token');
                this.$axios.put('/manager/notifications/publish',
                qs.stringify({
                    uid:this.pid
                }),
                {
            ...
            handleEdit(index, row) {
                ...
                this.$router.push({path:'/iupdataform',query:{uid:this.updatarr}});
            },
            ...
            deleteRow(){
                ...
                let token = localStorage.getItem('ms_token');
                this.$axios.delete("/manager/notifications",{
                    headers: {
                        'Authorization':token,
                    },
